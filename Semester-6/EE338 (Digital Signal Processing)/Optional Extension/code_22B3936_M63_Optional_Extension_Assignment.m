...
% Calculate order and LP_OmegaC for Group I (Chebyshev)
N1 = ceil(acosh(sqrt(D2/D1)) / acosh(LP_Omega_s1_min));
LP_OMegaC_1_min = 1/((D1)^(0.5/N1));
LP_OMegaC_1_max = LP_Omega_s1_min/((D2)^(0.5/N1));
LP_OmegaC_1 = (LP_OMegaC_1_min + LP_OMegaC_1_max)/2;

% Calculate order and LP_OmegaC for Group II (Chebyshev)
N2 = ceil(acosh(sqrt(D2/D1)) / acosh(LP_Omega_s2_min));
...
%% Design First Bandpass Filter (Chebyshev)
% Calculate Chebyshev poles for prototype LPF
epsilon1 = sqrt(D1);
eta1 = asinh(1/epsilon1) / N1;
poles_LHCP_1 = zeros(1, N1);
fprintf('Calculated poles for the prototype LPF 1 (Chebyshev):\n');
for k = 1:N1
    theta = (2*k - 1)*pi/(2*N1);
    sigma = sinh(eta1)*sin(theta);
    omega = cosh(eta1)*cos(theta);
    poles_LHCP_1(k) = -sigma + 1i*omega;
    fprintf('Pole (%d) = %.4f + %.4fi\n', k, real(poles_LHCP_1(k)), imag(poles_LHCP_1(k)));
end
...
title('LHCP Poles of the Prototype Low-Pass Filter 1 (Chebyshev)');
xlabel('Real');
ylabel('Imaginary');
grid on;
axis equal;
...
den_lpf1 = poly(poles_LHCP_1);

% % Adjust DC gain for Chebyshev Type I filter
if mod(N1, 2) == 0
    desired_H0 = 1 / sqrt(1 + epsilon1^2); % Desired DC gain for even-order Chebyshev
else
    desired_H0 = 1; % Desired DC gain for odd-order Chebyshev
end
num_lpf1 = den_lpf1(end) * desired_H0; % Scale numerator to adjust DC gain
...
%% Design Second Bandpass Filter (Chebyshev)
% Calculate Chebyshev poles for prototype LPF
epsilon2 = sqrt(D1);
eta2 = asinh(1/epsilon2) / N2;
poles_LHCP_2 = zeros(1, N2);
fprintf('Calculated poles for the prototype LPF 2 (Chebyshev):\n');
for k = 1:N2
    theta = (2*k - 1)*pi/(2*N2);
    sigma = sinh(eta2)*sin(theta);
    omega = cosh(eta2)*cos(theta);
    poles_LHCP_2(k) = -sigma + 1i*omega;
    fprintf('Pole (%d) = %.4f + %.4fi\n', k, real(poles_LHCP_2(k)), imag(poles_LHCP_2(k)));
end

% Plot the LHCP poles on the complex plane
figure;
plot(real(poles_LHCP_2), imag(poles_LHCP_2), 'x', 'MarkerSize', 10, 'LineWidth', 2, 'Color', [0, 0.5, 0]);
title('LHCP Poles of the Prototype Low-Pass Filter 2 (Chebyshev)');
xlabel('Real');
ylabel('Imaginary');
grid on;
axis equal;
...
% Normalize coefficients
den_lpf2 = poly(poles_LHCP_2);

% % Adjust DC gain for Chebyshev Type II filter
if mod(N2, 2) == 0
    desired_H0 = 1 / sqrt(1 + epsilon2^2); % Desired DC gain for even-order Chebyshev
else
    desired_H0 = 1; % Desired DC gain for odd-order Chebyshev
end
num_lpf2 = den_lpf2(end) * desired_H0; % Scale numerator to adjust DC gain
...
title('Magnitude Response of First Passband Chebyshev Filter');
...
xlabel('Frequency (KHz)');
ylabel('Magnitude');
title('Magnitude Response of Second Passband Chebyshev Filter');
yline(0.15, '--k', 'Tolerance 0.15', 'LabelHorizontalAlignment', 'left'); 
yline(0.85, '--k', 'Tolerance 0.85', 'LabelHorizontalAlignment', 'left');
...
xlabel('Frequency (KHz)');
ylabel('Magnitude');
title('Magnitude Response of Multi-Passband Chebyshev Filter');
yline(0.15, '--k', 'Tolerance 0.15', 'LabelHorizontalAlignment', 'left'); 
yline(0.85, '--k', 'Tolerance 0.85', 'LabelHorizontalAlignment', 'left');
